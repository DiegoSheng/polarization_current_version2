...
def current0(eltaOER,eltaORR,E,j0):
    if E>=1.23:
        ...
        jk=-1*j0*exp(-alpha*F*(E-1.23)/(R*T)+(1.23-eltaORR)/(kb*T))
        jd=-6 #taken from the reference
        j=(jd * jk)/(jd + jk)
    return j
...
plt.xticks( fontsize=16, fontproperties='Times New Roman',fontweight='bold')
plt.yticks(fontsize=16, fontproperties='Times New Roman',fontweight='bold')
plt.xlabel("Potential (V vs RHE)",fontdict={'family': 'Times New Roman', 'size': 16,'weight':'bold'})
plt.ylabel("j (mA/cm$^2$)",fontdict={'family': 'Times New Roman', 'size': 16,'weight':'bold'})
